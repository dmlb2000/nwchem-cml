# $Id: GNUmakefile,v 1.10 2007-06-12 22:00:23 manoj Exp $
#
# This makefile has to be processed by GNU make
#
EXPERIMENTAL=SGI64 LAPI64 CYGWIN HITACHI CATAMOUNT
SUPPORTED_TARGETS = SOLARIS SOLARIS64 SGI HPUX HPUX64 IBM IBM64 
SUPPORTED_TARGETS+= LAPI DECOSF LINUX LINUX64 CYGNUS CYGWIN INTERIX
SUPPORTED_TARGETS+= FUJITSU-VPP FUJITSU-VPP64 NEC MACX MACX64 BGL BGP $(EXPERIMENTAL)
...
# Makesure to avoid periods (i.e. dot) in GA_VERSION as that might(!) be
...
MAKESUBDIRS = for dir in $(SUBDIRS); do $(MAKE)  -C $$dir $@ || exit 1 ; done
    SUBDIRS = ma global tcgmsg-mpi LinAlg/lapack+blas tcgmsg $(ARMCI_DIR_NAME)/src pario ga++
  USE_ARMCI = yes
...
error: 
	@echo Cannot define MSG_COMMS=TCGMSG, when ARMCI_NETWORK=MPI-SPAWN
endif
endif
